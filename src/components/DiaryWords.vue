...
export default {
  ...
  data (){
    return {
      ...
      lst_rtc: ['Here is a dustbin filled with useless things.','Check the label list on the right side of this page.'],  // 用于存放当前所选文章的
    }
  },
  methods: {
    chs_rtl(vls){
      ...
      var tmp = [];
      for (var i=-2; i<3; i++){
        if(this.lst_key.indexOf(vls['key'])+i>=0&this.lst_key.indexOf(vls['key'])+i<this.lst_key.length){
    ...
    mnt_clk(){
      var tmp = [];
      for (var i=0; i<3; i++){tmp.push(this.lst_key[i]);};
      this.lst_clk = tmp;
