        ...
        <tr>
          <td>
            ...
            <div v-else style="text-align: center">
              ...
              <div>{{ rsl.words_result_num }} {{ rsl_nfo }}</div>
              <!--eslint-disable-next-line-->
              <a v-for="(wrd, index) in rsl_wrd" v-bind:key="index">{{ wrd.words }}</a>
            </div>
          </td>
...
export default {
  ...
  methods: {
    getPic() {
      ...
      document.querySelector("#img-pre").src = createObjectURL(
        pic_npt.files[0]
      ); // 指定<img src>实现图片预览
    ...
    sndPic() {
      ...
      rdr_fls.onload = function () {
        ...
        $.post(
          "http://www.xzzsmeadow.com:5000/vue/baiduocr",
          { image: this.result },
          function (result) {
